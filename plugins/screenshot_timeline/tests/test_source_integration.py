"""Integration test: source with mock helper, drive ticks, expect per-capture L1 items."""
...
async def test_capture_tick_produces_one_l1_item_per_capture(tmp_path: Path) -> None:
    ...
    source = source_mod.ScreenshotSource(
    ...
    try:
        ...
        await source.trigger_once("timer")
        items = await source.drain_pending_items()
        ...
        # Verify build_output produces a SourceOutput per item.
        output = await source.build_output(items[0])
...
async def test_phash_dedup_drops_near_identical_capture(tmp_path: Path) -> None:
    ...
    try:
        ...
        items_after_first = list(source._pending_items)
        assert len(items_after_first) == 1
        prior_phash = items_after_first[0]["phash"]
        source._last_phash_by_window[("com.apple.Safari", "Mock Window")] = prior_phash
        ...
        # through the source's last-phash table — when the second capture
        ...
        source.phash_dedup_threshold = 64
...
@pytest.mark.asyncio
async def test_source_active_window_timer_fires_capture(
    ...
    monkeypatch.setattr(source_mod, "screen_recording_status", lambda: "granted")
    monkeypatch.setattr(source_mod, "request_screen_recording", lambda: "granted")
    source = source_mod.ScreenshotSource(
        helper_argv=[sys.executable, str(_FIXTURE)],
        resources_root=tmp_path,
        session_db_path=tmp_path / "sessions.db",
...
async def test_collect_items_returns_per_capture_immediately(tmp_path: Path) -> None:
    ...
    try:
        ...
        result = await source.collect_items(context=None)  # type: ignore[arg-type]
        ...
        result2 = await source.collect_items(context=None)  # type: ignore[arg-type]
...
async def test_start_is_skipped_when_screen_recording_denied(
    ...
    PERMISSION_DENIED errors, so we refuse to start and leave the source idle
    until the user grants permission in System Settings and toggles the source
    again.
    """
    source_mod = _load("source")
    monkeypatch.setattr(source_mod, "screen_recording_status", lambda: "denied")
    ...
    try:
        ...
        assert source._orchestrator is None
        ...
        assert source._helper is not None and source._helper._proc is None
    finally:
        await source.stop()
...
async def test_build_output_prefers_ax_text_when_rich(tmp_path: Path) -> None:
    ...
    source_mod = _load("source")
    source = source_mod.ScreenshotSource(resources_root=tmp_path, session_db_path=tmp_path / "sessions.db")
    output = await source.build_output(_ax_item())
...
async def test_captures_are_l1_only_and_never_reach_l2(tmp_path: Path) -> None:
    ...
    assert source.memory_policy.cognition_eligible is False
    assert source.memory_policy.to_dict()["cognition_eligible"] is False
    assert source.memory_policy.ingest_target == "l1_only"  # still searchable at L1

    output = await source.build_output(_ax_item())
    assert source.l2_batch_policy(output) is None
