"""Screenshot timeline source — orchestrates triggers, helper, privacy, perceptual-hash dedup, and per-capture L1 emission.

Storage model: **one screenshot = one L1 event**.

Rationale: an earlier version of this source aggregated consecutive
...
class ScreenshotSource(Source):
    """Pull-sync source that emits one L1 event per screenshot capture."""

    source_id: str = "timeline.screenshot"
    ...
    memory_policy: SourceMemoryPolicy = SourceMemoryPolicy(cognition_eligible=False)
    ...
    async def start(self) -> None:
        ...
        Source has no host-driven start/stop hook (the host only calls
    ...
    async def _do_start(self) -> None:
        logger.info(
            "source.start.begin helper_argv=%s capture_scope=%s active_interval_s=%s full_screen_interval_min=%s",
        ...
        try:
            ...
            logger.debug("source.permission_request_failed", exc_info=True)
        ...
        try:
            ...
            logger.warning("source.permission_probe_failed", exc_info=True)
            status = "unknown"
        logger.info("source.start.permission_status=%s", status)
        if status == "denied":
            logger.warning(
                "source.permission_blocked status=%s — skipping helper start. "
        ...
        if self._helper is not None:
            ...
            logger.info("source.start.helper_spawned argv=%s", self.helper_argv)
        ...
        logger.info(
            "source.start.session_tracker_ready db=%s idle_threshold_s=%.0f",
        ...
        logger.info(
            "source.start.timers_installed active=%ss full_screen_timer=%s observer=%s",
        ...
        logger.info("source.start.complete — capture loop now live")
    ...
    async def collect_items(self, context: SourceSyncContext) -> SourceChangeBatch:
        ...
        Also handles lazy start: Source has no host-driven start hook,
        ...
        """
        logger.debug(
            "source.collect_items.called started=%s pending=%d",
        ...
        if not self._started:
            try:
                ...
                logger.exception("source.lazy_start_failed err=%r", exc)
                return SourceChangeBatch(changes=[])
            if not started:
                return SourceChangeBatch(changes=[])
    ...
    async def build_output(self, item: dict[str, Any]) -> SourceOutput:
        """Render one captured screenshot into a SourceOutput.
    ...
    async def extract_metadata(self, item: dict[str, Any]) -> SourceOutputMetadata:
        ...
        return SourceOutputMetadata(entities=entities)
    ...
    async def _trigger_once_current(self, trigger: str, generation: int) -> None:
        ...
        helper = self._helper
        if helper is None:
            logger.warning("trigger.no_helper — source wasn't fully started; skipping")
            return
        rid = new_capture_id()
