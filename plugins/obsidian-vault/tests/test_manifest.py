...
def test_manifest_is_valid_source_plugin() -> None:
    ...
    assert "source" in plugin["contribution_types"]
    # Opt-in by default (privacy-forward), like screenshot_timeline.
    assert plugin["default_settings"]["sources"]["obsidian_vault"]["enabled"] is False
