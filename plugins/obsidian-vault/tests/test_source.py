...
def test_build_output_maps_note_to_l1_fields() -> None:
    ...
    source = mod.ObsidianVaultSource(cognition_eligible=True, source_suffix="knowledge")
    out = asyncio.run(source.build_output(_sample_item()))
...
def test_memory_policy_differs_by_tier() -> None:
    mod = _load_source_module()
    knowledge = mod.ObsidianVaultSource(cognition_eligible=True, source_suffix="knowledge")
    search = mod.ObsidianVaultSource(cognition_eligible=False, source_suffix="search")
...
def test_extract_metadata_emits_host_contract_hints() -> None:
    mod = _load_source_module()
    source = mod.ObsidianVaultSource(cognition_eligible=True, source_suffix="knowledge")
    meta = asyncio.run(source.extract_metadata(_sample_item()))
...
def mod_sync_context(mod, vault: Path, last_cursor, settings):
    # Build a SourceSyncContext with the real SDK dataclass.
...
def test_collect_items_knowledge_tier_skips_search_and_excluded(tmp_path: Path) -> None:
    ...
    source = mod.ObsidianVaultSource(
        cognition_eligible=True, source_suffix="knowledge",
        vault_path=str(tmp_path),
        exclude_folders=[".obsidian"], cognition_exclude_folders=["Clippings"],
    )
    result = asyncio.run(source.collect_items(mod_sync_context(mod, tmp_path, None, {})))
...
def test_collect_items_incremental_via_cursor(tmp_path: Path) -> None:
    ...
    result = asyncio.run(source.collect_items(mod_sync_context(mod, tmp_path, '{"mtime":2000.0,"object_id":""}' , {})))
    assert [change.payload for change in result.changes] == []

    # A fresh file (current mtime) is picked up.
    new = tmp_path / "Projects" / "New.md"
    new.write_text("# New\nnew\n", encoding="utf-8")
    result2 = asyncio.run(source.collect_items(mod_sync_context(mod, tmp_path, '{"mtime":2000.0,"object_id":""}' , {})))
