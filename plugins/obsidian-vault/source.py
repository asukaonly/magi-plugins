# source.py
"""Obsidian vault timeline source."""
...
class ObsidianVaultSource(Source):
    """Pull-sync source that ingests Obsidian markdown notes.

    Instantiated twice by the plugin: a ``knowledge`` instance
    (``cognition_eligible=True``) and a ``search`` instance
    (``cognition_eligible=False``). ``SourceMemoryPolicy`` is per-instance, which is
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.source_id = f"timeline.obsidian_vault.{source_suffix}"
        # Distinct source_type per tier. The host resolves/schedules/cursors sources by
        # (plugin_id, source_type) and resolve_source() is first-match-wins, so two
        # sources sharing a source_type collide (only the first ever runs). Distinct
        # source_types give each tier its own schedule + cursor.
        self.source_type = (
            "obsidian_vault" if source_suffix == "knowledge" else f"obsidian_vault_{source_suffix}"
        )
        self.display_name = "Obsidian Vault"
        self._tier = source_suffix
        self._vault_path = vault_path
        self._exclude_folders = exclude_folders or []
        self._cognition_exclude_folders = cognition_exclude_folders or []
        self.memory_policy = SourceMemoryPolicy(
    ...
    def _resolve_settings(self, context: SourceSyncContext) -> dict[str, Any]:
        """Merge constructor defaults with live plugin settings for this source."""
        sources = context.plugin_settings.get("sources", {})
        live = sources.get("obsidian_vault", {}) if isinstance(sources, dict) else {}
        return {
