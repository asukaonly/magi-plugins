"""Normalization helpers shared by photo source plugins."""
...
def build_session_entity_hints(session: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    * ``hardware``  鈥?the camera identity used during the session.
...
def build_session_relation_candidates(session: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    * ``user:self -OWNS-> hardware``  when device identity is known.
        ...
        candidates.append(
            {
                ...
                "subject_type": "user",
                "predicate": "OWNS",
                "object_id": f"hardware:{device_slug}",
