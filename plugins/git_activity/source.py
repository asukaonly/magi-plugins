"""Timeline source for Git Activity."""
...
class GitActivitySource(Source):
    """Timeline source for Git Activity data."""
    ...
    async def extract_metadata(self, item: dict) -> SourceOutputMetadata:
    ...
    async def collect_items(self, context: SourceSyncContext) -> SourceChangeBatch:
        ...
        repos = source_settings.get("repos", self._repos)
        if not repos:
        ...
        # Get settings
        lookback_days = int(source_settings.get("initial_sync_lookback_days", 30))

        initial_policy = source_settings.get("initial_sync_policy", "lookback_days")
        ...
        session_window_seconds = max(
            60.0,
            float(source_settings.get("session_window_minutes", DEFAULT_SESSION_WINDOW_SECONDS // 60)) * 60.0,
        )
        max_messages = max(0, int(source_settings.get("max_messages_per_session", DEFAULT_MAX_MESSAGES_PER_SESSION)))

        # Get filter
        msg_filter = self._get_filter(source_settings)
    ...
    async def build_output(self, item: dict) -> SourceOutput:
        """Build a SourceOutput from a git activity item."""
        # Normalize the item
