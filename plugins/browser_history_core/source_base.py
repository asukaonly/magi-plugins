"""Shared browser history timeline source base."""
...
class BrowserHistoryReaderLike(Protocol):
    """Reader protocol shared by Chromium and Firefox readers."""
...
class BaseBrowserHistoryTimelineSource(Source):
    """Shared pull-sync source backed by a local browser history SQLite database."""

    source_id = "timeline.browser_history"
    ...
    def __init__(
        self,
        *,
        reader: BrowserHistoryReaderLike,
    ...
    async def collect_items(self, context: SourceSyncContext) -> SourceChangeBatch:
        ...
        source_path = str(source_settings.get("source_path") or self.source_path or "")
        ...
        return self._build_output(
            ...
            activity=self._build_activity(
                source=self._build_activity_facet(
                    ...
                    i18n_key=f"activity.source.{self.browser_code}",
                    fallback=self.browser_label,
                    embedding_fallback=self.browser_label,
                ),
...
def _compile_domain_patterns(values: Any) -> list[re.Pattern[str]]:
    ...
    for raw in values:
        ...
        try:
            ...
            logger.warning("Ignoring invalid browser domain filter %r: %s", text, exc)
    return patterns
