...
def chrome_time_to_unix_seconds(value: int | float | str | None) -> float:
    """Convert Chromium microseconds since 1601 into Unix seconds."""
...
def firefox_time_to_unix_seconds(value: int | float | str | None) -> float:
    """Convert Firefox microseconds since Unix epoch into Unix seconds."""

    if value in (None, "", 0, "0"):
        return 0.0
    numeric = float(value)
    return max(0.0, numeric / 1_000_000.0)
...
def parse_title_entities(
    ...
    """Extract structured entity hints from a browser page title."""
