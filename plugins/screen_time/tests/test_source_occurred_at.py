...
def _load_source_module() -> ModuleType:
    """Load ``source.py`` without going through the plugin package import.

    The plugin package depends on a private state store and watcher whose
    full import chain is platform-dependent. Loading the source module by
    file path keeps this test self-contained and OS-agnostic.
    """
    plugin_dir = Path(__file__).resolve().parents[1]
    # ``source.py`` does ``from ._watcher import ...`` etc., so we have to
