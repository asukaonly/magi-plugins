"""Timeline source for Calendar data."""
...
class CalendarTimelineSource(Source):
    """Timeline source for Calendar data."""
    ...
    async def collect_items(self, context: SourceSyncContext) -> SourceChangeBatch:
        ...
        # Get settings
        lookback_days = source_settings.get("lookback_days", 30)
        recurring_expansion_days = source_settings.get("recurring_expansion_days", 30)
        selected_calendar_ids = source_settings.get("selected_calendar_ids", [])
        ...
        logger.info(
            f"Calendar source collected events source_type={self.source_type} manual={context.manual} "
        ...
        for event in events:
            ...
            items.append(item)
            logger.info(
                f"Calendar source prepared source item event_id={item['event_id']} title={item['title']} "
    ...
    async def build_output(self, item: dict) -> SourceOutput:
        """Build a SourceOutput from a calendar event item."""
        ...
        logger.info(
            f"Calendar source built output event_id={event.event_id} "
