...
"""Agent history source: ingest the user's OWN turns from local agent transcripts,
...
tag, which Rule 1 extracts as the user's own facts. A default source
(``author_type="external"`` -> ``[EXTERNAL]``) would NOT be extracted -- so this
policy is the whole point of the source.

Sync model: the plugin registers one pull-sync source per agent. Each source
...
class CodingAgentHistorySource(Source):
    """Pull-sync source that ingests the user's own agent transcript turns."""

    source_id = "timeline.agent_history"
    ...
    def __init__(
        ...
        *,
        ...
        self.source_id = f"timeline.{self.source_type}"
    ...
    def _resolve_settings(self, context: SourceSyncContext) -> dict[str, Any]:
        """Pull this source's settings out of the plugin settings tree."""
        settings = context.plugin_settings
        sources = settings.get("sources", {}) if isinstance(settings, dict) else {}
        cfg = sources.get(self.source_type, {}) if isinstance(sources, dict) else {}
