"""Timeline source for Terminal History."""
...
class TerminalHistorySource(Source):
    """Timeline source for Terminal History data."""
    ...
    async def collect_items(self, context: SourceSyncContext) -> SourceChangeBatch:
        ...
        # Get settings
        lookback_days = int(source_settings.get("initial_sync_lookback_days", 7))
        dedup_window_seconds = int(source_settings.get("dedup_window_seconds", 60))
        ...
        # Get filter
        cmd_filter = self._get_filter(source_settings)
    ...
    async def build_output(self, item: dict) -> SourceOutput:
        """Build a SourceOutput from a terminal command item."""
        # Normalize the item
