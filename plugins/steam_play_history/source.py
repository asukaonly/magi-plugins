"""Timeline source for Steam play history."""
...
class SteamPlayHistoryTimelineSource(Source):
    """Pull-sync source backed by local Steam files."""
    ...
    async def _collect_items(self, context: SourceSyncContext) -> SourceChangeBatch:
        settings = _source_settings(context.plugin_settings)
...
def _source_settings(plugin_settings: dict[str, Any]) -> dict[str, Any]:
    sources = plugin_settings.get("sources", {}) if isinstance(plugin_settings, dict) else {}
    if not isinstance(sources, dict):
        return {}
    value = sources.get("steam_play_history", {})
