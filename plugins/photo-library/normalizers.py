...
def build_entity_hints(item: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    Only the location is treated as a first-class entity. Cameras stay in
    provenance for L2 fact extraction but are not surfaced as graph nodes —
    "user owns a camera" should be derived from many photos, not duplicated
    onto every event.
    """
...
def build_relation_candidates(item: dict[str, Any]) -> list[dict[str, Any]]:
    """Generate relation candidates for a photo item.

    Only emits ``photo -RELATED_TO- location`` when GPS is available.
    Higher-level facts ("user visited Hangzhou", "user uses Sony α7 IV")
    are derived during L2 batch synthesis from per-photo records.
    """
