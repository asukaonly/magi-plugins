...
# Geo grid size for L2 batching: ~11km, large enough to keep one outing in a
# single bucket but small enough to keep distinct cities apart.
_GEO_CELL_DEGREES = 0.1
...
class PhotoLibraryTimelineSensor(SensorBase):
    ...
    relation_edge_whitelist = ("RELATED_TO",)
    ...
    def l2_batch_policy(self, output: SensorOutput) -> L2BatchPolicy | None:
        """Group photos by local date + ~11km geo cell.

        A "trip" or "outing" is the natural unit for L2 fact extraction —
        same day, same area. Photos without GPS go into a per-day "nogps"
        bucket so they still get batched, but kept apart from located ones.
        """
        lat = output.provenance.get("latitude")
        lon = output.provenance.get("longitude")
        ...
        if lat is not None and lon is not None:
            cell_lat = round(float(lat) / _GEO_CELL_DEGREES) * _GEO_CELL_DEGREES
            cell_lon = round(float(lon) / _GEO_CELL_DEGREES) * _GEO_CELL_DEGREES
            geo_key = f"{cell_lat:.2f},{cell_lon:.2f}"
        else:
            geo_key = "nogps"

        owner = f"{self.source_type}:{day}:{geo_key}"
        return L2BatchPolicy(
            owner=owner,
            catch_up_owner=f"{self.source_type}:catchup:{geo_key}",
            max_events=20,
            min_ready_events=3,
    ...
    async def build_output(self, item: dict[str, Any]) -> SensorOutput:
        ...
        burst_count = int(item.get("burst_count") or 0)

        # Compose a single-line summary that carries every retrieval-relevant
        # signal. The image content block stays as the only attached blob.
        summary_parts: list[str] = []
        if camera:
            summary_parts.append(camera)
        if params:
            summary_parts.append(params)
        if location_name:
            summary_parts.append(f"@ {location_name}")
        if burst_count > 1:
            summary_parts.append(f"(burst x{burst_count})")
        summary = " · ".join([filename] + summary_parts) if summary_parts else filename
        ...
        # Slim provenance: keep only what L2 fact extraction or downstream
        # retrieval will actually use.
        ...
        if burst_count > 1:
            provenance["burst_count"] = burst_count
