"""Generic local documents timeline source."""
...
class LocalDocumentsSource(Source):
    """Pull-sync source that ingests generic local text documents."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.source_id = f"timeline.local_documents.{source_suffix}"
        self.source_type = "local_documents" if source_suffix == "knowledge" else "local_documents_search"
    ...
    def _resolve_settings(self, context: SourceSyncContext) -> dict[str, Any]:
        ...
        live = sources.get("local_documents", {}) if isinstance(sources, dict) else {}
        return {
